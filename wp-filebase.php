...
/*
  ...
  Plugin URI:  https://www.hadithemes.com/wp-filebase-pro-extended/
  ...
  Author URI:  https://www.hadithemes.com/
...
if (!defined('WPFB')) {
    ...
    function wpfb_call($cl, $fnc, $params = null, $is_args_array = false)
    {
        $cln = 'WPFB_' . $cl;
        $fnc = array($cln, $fnc);
        return (class_exists($cln) || wpfb_loadclass($cl))
            ? ($is_args_array
                ? call_user_func_array($fnc, $params)
                : (func_num_args() > 2 ? call_user_func($fnc, $params) : call_user_func($fnc)))
            : null;
    }
